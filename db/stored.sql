...
BEGIN
  OPEN RESULT FOR
  SELECT USER_ID, USERNAME, HASH_PASS, HASH_SALT, LOGIN_TYPE
  FROM USER_AUTHEN
  WHERE USERNAME = LOWER(P_USERNAME)
    AND STATUS = 1
  ;
...
BEGIN
  ...
  -- store message
  INSERT INTO MESSAGE (MESSAGE_ID, MESS_BODY, STATUS, FROM_USER_ID, CONVERSATION_ID, DATE_SENT, CLIENT_MESSAGE_ID)
  VALUES (SEQ_MESS_ID.NEXTVAL, P_MESS_BODY, 1, FROM_ID, CONV_ID, P_DATE_SENT, P_CLIENT_MESSAGE_ID)
...
END;
/
create or replace PROCEDURE UPDATE_MESSAGE_USER
(
  P_MESSAGE_ID NUMBER
, P_USER_ID NUMBER
, P_DATE_SENT DATE
, P_STATUS NUMBER
  ...
  MERGE INTO MESSAGE_USER D
  USING DUAL S
  ON (D.USER_ID = P_USER_ID AND D.MESSAGE_ID = P_MESSAGE_ID)
  WHEN MATCHED THEN UPDATE
    SET D.STATUS = P_STATUS
      , D.DATE_SENT = CASE P_STATUS
                        WHEN 1 THEN P_DATE_SENT
                        ELSE D.DATE_SENT
                      END  
      , D.DATE_RECEIVED = CASE P_STATUS
                        WHEN 2 THEN P_DATE_SENT
                        ELSE D.DATE_RECEIVED
                      END  
                        
  WHEN NOT MATCHED THEN
    INSERT (MESSAGE_ID, USER_ID, STATUS, DATE_SENT, DATE_RECEIVED)
  ...
  -- update main status if all status = 2
  UPDATE MESSAGE
  SET STATUS = 2
  WHERE MESSAGE_ID = P_MESSAGE_ID
    AND 2 = (SELECT MIN(STATUS) FROM MESSAGE_USER WHERE MESSAGE_ID = P_MESSAGE_ID)
  ;  
END;
/
create or replace FUNCTION GET_NEW_MESSAGE
(
  P_USERNAME IN VARCHAR2
) RETURN SYS_REFCURSOR IS
  ...
  V_USER_ID_TO NUMBER;
BEGIN
  SELECT USER_ID INTO V_USER_ID_TO FROM USER_AUTHEN WHERE USERNAME = LOWER(P_USERNAME);
  ...
  FROM MESSAGE M
    ...
    JOIN USER_CONV UC ON UC.CONVERSATION_ID = C.CONVERSATION_ID
    
    JOIN USER_AUTHEN UF ON UF.USER_ID = M.FROM_USER_ID

    LEFT JOIN MESSAGE_USER MU ON 1 = 1
      AND MU.MESSAGE_ID = M.MESSAGE_ID
      AND MU.USER_ID = V_USER_ID_TO
      
  WHERE 1 = 1
    AND M.STATUS = 1
    AND UC.USER_ID = V_USER_ID_TO
    AND UF.USER_ID != V_USER_ID_TO
    AND (MU.MESSAGE_ID IS NULL OR MU.STATUS = 1)
  ;
